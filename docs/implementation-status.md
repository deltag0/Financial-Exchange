...
- Per-command events use an exactly planned, dynamically allocated vector, but the adopted
  4,096-event limit and independently reserved rejection result are not enforced. No atomic event
  handoff or partition backpressure path exists.
