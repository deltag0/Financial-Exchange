...
void FixTask::toApp(FIX::Message &message, const FIX::SessionID &) noexcept {
  std::cout << "[FixTask] -> Sending App Data: " << message.toString()
            << std::endl;
}

void FixTask::fromAdmin(const FIX::Message &message,
                        const FIX::SessionID &) noexcept {
  std::cout << "[FixTask] <- Received Admin (Raw): "
            << message.toString().substr(0, 30) << "..." << std::endl;
}

void FixTask::fromApp(const FIX::Message &message,
...
void FixTask::sendFixMessage(FIX::Message &message,
...
void FixTask::sendSequencerMessage(sequencer::sequenceMessage &message) {
  if (!mq_shards[message.shard_id]->push(message)) {
    std::cerr << "[FixTask] Failed to send message to sequencer shard "
              << (int)message.shard_id << std::endl;
  }
}
...
void FixTask::run() {
  while (true) {
    ...
    for (int i = 0;
         i < EXT_BURST_MESSAGES && !internalQueues.fixMessageQueue->empty();
         ++i) {
