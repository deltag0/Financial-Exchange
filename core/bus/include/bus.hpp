...
public:
    ...
    bool write(const sequencer::sequenceMessage &value) {
        return writeImpl(value);
    }

    bool write(sequencer::sequenceMessage &&value) {
        return writeImpl(std::move(value));
    }
...
private:
    ...
    /*
    Note that I think there would be a way to always keep track of the minimum cursor using a linked list and map with
    the pointers poitning to their node then on a read, it's an O(1) operation to update the minimum cursor
    */
