...
Follow `AGENTS.md`. This file is a tool-specific review adapter and does not define exchange behavior,
architecture, or implementation status.

Review changes against the current implementation and `docs/exchange-rules.md`. Do not assume that
the proposed API, validation, observability, or exchange behavior is already implemented.
...
5. For new API routes, verify input validation against `.claude/rules/api-conventions.md` and the
   adopted rules in `docs/exchange-rules.md`. Flag conflicts as unresolved decisions; do not silently
   choose a representation or identifier policy.
